...
// Also emit the CSV as a TypeScript string module. The browser build (Vite) and
// the server bundle (esbuild, in server/) both import this instead of a
// Vite-only `?raw` import, so the exact same data loads in both environments.
const escaped = csv.replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\$\{/g, '\\${')
const tsModule =
  '// AUTO-GENERATED by scripts/generate-ledger.mjs — do not edit by hand.\n' +
  '// The ledger CSV as a string module, loadable by both the Vite client build\n' +
  '// and the Node/esbuild server bundle (no Vite-only ?raw import).\n' +
