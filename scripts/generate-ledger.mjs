// Deterministic (seeded) generator for the ledger dataset. Run: node scripts/generate-ledger.mjs
...
const OUT_TS = join(__dirname, '..', 'src', 'data', 'ledgerCsv.ts')
...
// Unique reference numbers, so within-month recurring lines aren't false duplicates; the planted dupe below reuses invoice 8841 on purpose.
let refSeq = 10420
...
MONTHS.forEach((month, mi) => {
  ...
  // Rent — indexation steps. June 2026 is deliberately omitted (the planted "missing rent" gap).
  ...
  add({ date: day(month, 28), description: 'Bank account & card fees', counterparty: 'Danske Bank A/S', category: 'Fees', amount: -round50(between(250, 600)) })
  for (let i = 0; i < 1 + Math.floor(rand() * 3); i++) {
    add({
...
// Also emit the CSV as a TS string module, so both the Vite client and esbuild server bundle import identical data (no ?raw).
const escaped = csv.replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\$\{/g, '\\${')
const tsModule =
  '// AUTO-GENERATED by scripts/generate-ledger.mjs — do not edit by hand.\n' +
  `export default \`${escaped}\`\n`
writeFileSync(OUT_TS, tsModule, 'utf8')
console.log(`Wrote ${rows.length} rows to ${OUT} and ${OUT_TS}`)
